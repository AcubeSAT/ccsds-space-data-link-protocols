...
namespace CCSDSDataLinkLayer::DefsAndUtils {
    ...
    enum class TcFrameProcessingStage : uint8_t {
        // Tx chain
        PROCESSED_BY_PACKET_PROCESSING = 0x0,
        PROCESSED_BY_SECURITY_TX = 0x1,
        PROCESSED_BY_VC_GENERATION_TX = 0x2,
        PROCESSED_BY_ALL_FRAMES_GENERATION = 0x3,
        // Rx chain
        PROCESSED_BY_ALL_FRAMES_RECEPTION = 0x4,
        PROCESSED_BY_FARM = 0x5,
    ...
    /**
     * MSCID = | TFVN (2 bits) | SCID (10 bits) |
     */
    inline uint16_t buildMscid(const TransferFrameVersionNumber tfvn, const uint16_t scid) {
        return (static_cast<uint16_t>(tfvn) << 10U) | (scid & 0x03FF);
    }

    /**
     * GVSCID = | TFVN (2 bits) | SCID (10 bits) | VCID (6 bits) |
     */
    inline uint32_t buildGvcid(const TransferFrameVersionNumber tfvn, const uint16_t scid, const uint8_t vcid) {
        return (static_cast<uint32_t>(tfvn) << 16U) | (static_cast<uint32_t>(scid & 0x03FF) << 6U) | (vcid & 0x3F);
    }

    /**
     * GMAPID = | TFVN (2 bits) | SCID (10 bits) | VCID (6 bits) | MAP ID (6 bits) |
     */
    inline uint32_t buildGmapid(const TransferFrameVersionNumber tfvn, const uint16_t scid, const uint8_t vcid,
                                const uint8_t mapid) {
        return (static_cast<uint32_t>(tfvn) << 22U) | (static_cast<uint32_t>((scid & 0x03FF)) << 12U) | (
                   (vcid & 0x3F) << 6U) | (mapid & 0x3F);
    }

    inline uint16_t extractScidFromMcid(const uint16_t mscid) {
        return mscid & 0x03FFU;
    }

    inline uint16_t extractScidFromGvcid(const uint16_t gvcid) {
        return (gvcid >> 6U) & 0x03FFU;
    }

    inline uint16_t extractScidFromGmapid(const uint16_t gmapid) {
        return (gmapid >> 12U) & 0x03FFU;
    }

    inline uint8_t extractVcidFromGvcid(const uint16_t gvcid) {
        return gvcid & 0x3FU;
    }

    inline uint8_t extractVcidFromGmapid(const uint16_t gmapid) {
        return (gmapid >> 6U) & 0x3FU;
    }

    inline uint8_t extractMapidFromGmapid(const uint16_t gmapid) {
        return gmapid & 0x3FU;
    }
    ...
    */
    ...
    enum class SecurityAssociationServiceType : uint8_t {
        AUTHENTICATION,
        ENCRYPTION,
