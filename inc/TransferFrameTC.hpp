...
public:
	...
	TransferFrameTC(uint8_t *transfer_frame, uint16_t transfer_frame_length, uint8_t segHdr, uint8_t gvcid, uint8_t mapid, uint16_t sduid,
             ServiceType serviceType, bool segHdrPresent, TransferFrameType t=TC):  TransferFrame(t, transfer_frame_length,transfer_frame),
	         hdr(transfer_frame), segHdr(segHdr), gvcid(gvcid), mapid(mapid), sduid(sduid), serviceType(serviceType),
	         transferFrameSeqNumber(0), ack(false), toBeRetransmitted(false), transferFrameVersionNumber(0)
	          {
		data=&transfer_frame[5 + 1 * segHdrPresent];
	}
