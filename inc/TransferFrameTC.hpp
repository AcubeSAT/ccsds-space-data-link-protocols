...
public:
	...
	friend bool operator==(const TransferFrameTC& packet1, const TransferFrameTC& packet2) {
		if (packet1.auxiliaryTransferFrameLength != packet2.auxiliaryTransferFrameLength) {
			return false;
		}
		for (uint16_t i = 0; i < packet1.auxiliaryTransferFrameLength; i++) {
			if (packet1.getTransferFrameData()[i] != packet2.getTransferFrameData()[i]) {
				return false;
			}
		}
		return true;
	...
	uint8_t* getTransferFrameData() const {
	...
	void setTransferFrameData(uint8_t* transferFrameData) {
		transferFrameData = transferFrameData;
	}

	void setPacketLength(uint16_t packet_length) {
		transferFrameData[2] = ((virtualChannelId() & 0x3F) << 2) | (auxiliaryTransferFrameLength & 0x300 >> 8);
		transferFrameData[3] = auxiliaryTransferFrameLength & 0xFF;
	}
	...
	TransferFrameTC(uint8_t* transferFrameData, uint16_t frameLength, uint8_t gvcid, ServiceType serviceType, bool segHdrPresent,
	                PacketType t = TC)
	    : TransferFrame(t, frameLength, transferFrameData), hdr(transferFrameData), serviceType(serviceType), ack(false),
		...
		transferFrameData[0] = (bypassFlag << 6) | (ctrlCmdFlag << 5) | ((SpacecraftIdentifier & 0x300) >> 8);
		transferFrameData[1] = SpacecraftIdentifier & 0xFF;
		transferFrameData[2] = ((gvcid & 0x3F) << 2) | (frameLength & 0x300 >> 8);
		transferFrameData[3] = frameLength & 0xFF;
	}

	TransferFrameTC(uint8_t* transferFrameData, uint16_t frameLength, PacketType t = TC)
	    : TransferFrame(PacketType::TC, frameLength, transferFrameData), hdr(transferFrameData), transmit(false){};
	/**
	 * Calculates the CRC code
	 * @see p. 4.1.4.2 from TC SPACE DATA LINK PROTOCOL
	 */
	 uint16_t calculateCRC(const uint8_t* transferFrameData, uint16_t len) override {
		uint16_t crc = 0xFFFF;

		// calculate remainder of binary polynomial division
		for (uint16_t i = 0; i < len; i++) {
			crc = crc_16_ccitt_table[(transferFrameData[i] ^ (crc >> 8)) & 0xFF] ^ (crc << 8);
		}
