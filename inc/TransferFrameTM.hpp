...
struct TransferFrameTM : public TransferFrame {
	...
	TransferFrameTM(uint8_t* transferFrameData, uint16_t packetLength, uint8_t virtualChannelFrameCount, uint16_t vcid,
	    ...
	    : TransferFrame(type, packetLength, transferFrameData), hdr(transferFrameData), scid(scid), eccFieldExists(eccFieldExists),
	      firstHeaderPointer(firstHeaderPointer) {
		// Transfer Frame Version Number + Spacecraft Id
		transferFrameData[0] = SpacecraftIdentifier & 0xE0 >> 4;
		// Spacecraft  Id + Virtual Channel ID + Operational Control Field
		transferFrameData[1] = ((SpacecraftIdentifier & 0x0F) << 4) | ((vcid & 0x7) << 1);
		// Master Channel Frame Count is set by the MC Generation Service
		transferFrameData[2] = 0;
		transferFrameData[3] = virtualChannelFrameCount;
		// Data field status. TransferFrame Order Flag and Segment Length ID are unused
		transferFrameData[4] = (transferFrameSecondaryHeaderPresent << 7) | (static_cast<uint8_t>(syncFlag << 6)) |
		            (segmentationLengthId << 3) | (firstHeaderPointer & 0x700 >> 8);
		transferFrameData[5] = firstHeaderPointer & 0xFF;
	}
	...
	TransferFrameTM(uint8_t* transferFrameData, uint16_t packetLength, uint8_t virtualChannelFrameCount, uint16_t vcid,
		...
		transferFrameData[0] = SpacecraftIdentifier & 0xE0 >> 4;
		// Spacecraft  Id + Virtual Channel ID + Operational Control Field
		transferFrameData[1] = ((SpacecraftIdentifier & 0x0F) << 4) | ((vcid & 0x7) << 1) | 1;
		// Master Channel Frame Count is set by the MC Generation Service
		transferFrameData[2] = 0;
		transferFrameData[3] = virtualChannelFrameCount;
		// Data field status. TransferFrame Order Flag and Segment Length ID are unused
		transferFrameData[4] = (transferFrameSecondaryHeaderPresent << 7) | (static_cast<uint8_t>(syncFlag << 6)) |
		            (segmentationLengthId << 3) | (firstHeaderPointer & 0x700 >> 8);
		transferFrameData[5] = firstHeaderPointer & 0xFF;
		uint8_t* ocfPointer = transferFrameData + auxiliaryTransferFrameLength - 4 - 2 * eccFieldExists;
	...
	TransferFrameTM(uint8_t* transferFrameData, uint16_t frameLength, bool eccFieldExists)
	    : TransferFrame(PacketType::TM, frameLength, transferFrameData), hdr(transferFrameData),
