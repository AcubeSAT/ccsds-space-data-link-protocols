...
VirtualChannelAlert VirtualChannel::storeVC(TransferFrameTC* transfer_frame {
	...
	txUnprocessedTransferFrameListBufferTC.push_back(transfer_frame);
...
MasterChannelAlert MasterChannel::storeOut(TransferFrameTC* transfer_frame) {
...
MasterChannelAlert MasterChannel::storeTransmittedOut(TransferFrameTC* transfer_frame) {
	...
	txToBeTransmittedFramesAfterAllFramesGenerationList.push_back(transfer_frame);
	ccsdsLog(Tx, TypeMasterChannelAlert, NO_MC_ALERT);
    return MasterChannelAlert::NO_MC_ALERT;
}
