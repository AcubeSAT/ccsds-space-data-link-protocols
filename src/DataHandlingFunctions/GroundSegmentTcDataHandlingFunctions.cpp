...
namespace CCSDSDataLinkLayer {
    ...
    etl::expected<void, ServiceChannelNotification> GroundSegmentTcDataHandling::storePacket(
        ...
        if (chanVariant.is_type<VirtualChannelGsTc>()) {
            VirtualChannelGsTc& vcChan = etl::get<VirtualChannelGsTc>(chanVariant);
            ...
            MAPChannelGs& mapChan = etl::get<MAPChannelGs>(chanVariant);
	    ...
	    while (!packetLengths->isEmpty()) {
	    	...
	    	if (!framesAfterPacketProcessing->isEmpty()) {
	    		...
	    		if (dataFieldContent == Defs::DataFieldContent::PACKET &&
					blockingAllowed &&
					prevFrame->getSequenceFlag() == Defs::SequenceFlag::NoSegmentation &&
	    	...
	    	if (packetLength > maxTransferFrameDataFieldLength &&
			    ...
			    for (uint16_t i = 0; i < amountNewFrames; i++) {
				    ...
				    Defs::SequenceFlag seqFlag = Defs::SequenceFlag::SegmentationMiddle;
				    if (i == 0) {
					    seqFlag = Defs::SequenceFlag::SegmentationStart;
				    } else if (i == amountNewFrames - 1) {
					    seqFlag = Defs::SequenceFlag::SegmentationEnd;
				    }
	    		...
	    		// create frame object and allocate memory
	    		uint8_t* frameData = Objects::frameOctetPool.allocateBlock(frameOctetsToAllocate);
	...
	etl::expected<void, ServiceChannelNotification> GroundSegmentTcDataHandling::applySDLSSecurity(
    	...
    	if (vcChan.getsegmentHeaderPresent()) {
    		...
    		uint8_t mapid = frameTcPtr->getMapId().value();
    		Defs::MapidVcidScidKey key = constructMapidVcidScidKey(mapid, vcChan.getVcid(), vcChan.getParentScid());
    	...
    	if (associatedSdlsSpi.has_value()) {
		    ...
		    uint16_t transferFrameDataFieldLength = frameTcPtr->getFrameLength() -
		                                            Defs::TcPrimaryHeaderSize -
    		...
    		if (const auto status = sa.processSecurityTC(frameTcPtr, transferFrameDataFieldLength);
    			...
    			vcChan.channelMutex.unlock();
    			return etl::unexpected(ServiceChannelNotification::SLDS_ERROR);
    		}
    	}
	...
	etl::expected<void, ServiceChannelNotification> GroundSegmentTcDataHandling::virtualChannelGeneration(
    	...
    	bool encounteredFopSignal = false; // let the user no if no fop signals were encountered in this function call
