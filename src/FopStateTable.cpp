        ...
        if (!transferFduSignalQueue.empty()) {
            ...
            if (fduTransferSignal.serviceType == ServiceType::TYPE_AD) {
                if (waitQueueFOP.empty()) {
                    ...
                    eventCode = 20;
                    transferNotificationSignalQueue.push(
                            TransferNotificationSignal(TransferNotificationType::REJECT_RESPONSE_TO_TRANSFER_FDU, fduTransferSignal.frame));
                }
                ...
                if (bdOut) {
                    ...
                    eventCode = 21;
                    transferNotificationSignalQueue.push(
                            TransferNotificationSignal(TransferNotificationType::ACCEPT_RESPONSE_TO_TRANSFER_FDU, fduTransferSignal.frame));
        ...
        if (!directiveRequestSignalQueue.empty()) {
            ...
            switch (directiveRequestSignal.directiveType) {
                ...
                case DirectiveRequestType::RESUME_AD_SERVICE:
                    // E30 to E34
                    if (suspendState == SuspendVariableState::NOT_SUSPENDED) {
                        eventCode = 30;
                        directiveNotificationSignalQueue.push(
                                DirectiveNotificationSignal(directiveRequestSignal.requestIdentifier,
                                                            DirectiveNotificationType::REJECT_RESPONSE_TO_DIRECTIVE));
                ...
                case DirectiveRequestType::SET_NEW_VS:
                    ...
                    // E35 rev. B
                    eventCode = 35;
                    if ((state == FOPState::INITIAL) && (suspendState == SuspendVariableState::NOT_SUSPENDED)) {
                        directiveNotificationSignalQueue.push(
                                DirectiveNotificationSignal(directiveRequestSignal.requestIdentifier,
                                                            DirectiveNotificationType::ACCEPT_RESPONSE_TO_DIRECTIVE));
                        transmitterFrameSeqNumber = static_cast<uint8_t>(directiveRequestSignal.directiveQualifier.value());
