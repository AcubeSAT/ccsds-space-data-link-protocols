...
uint16_t PacketTM::calculate_crc_tm(uint8_t *packet, uint16_t len) {
	uint16_t crc = 0xFFFF;
...
void PacketTM::append_crc_tm() {
	uint16_t len = packet_length() - 2;
	uint16_t crc = calculate_crc_tm(packet_data(), len);
