...
namespace CCSDSDataLinkLayer {
    ServiceChannelNotification FrameMaker::transmitChain(uint8_t vid) {
        ...
        numberOfPacketsSent +=
                initialPacketsNum - (PacketBufferTmSize - serviceChannel->availablePacketLengthBufferTxTM(vid));

        // (secondary header) and CLCW insertion for all created frames
        do {
            ser = serviceChannel->mcGenerationRequestTxTM();
        } while (ser == ServiceChannelNotification::NO_SERVICE_EVENT);

        // error control encoding for all frames
        while (serviceChannel->allFramesGenerationRequestTxTM(frameTarget) == ServiceChannelNotification::NO_SERVICE_EVENT) {
