...
namespace CCSDSDataLinkLayer {
    ...
    etl::expected<void, ServiceChannelNotification>
    	...
    	etl::optional<DefsAndUtils::PacketVersionNumber> pvn = DefsAndUtils::getPacketVersionNumber(packetSource[0]);

    	// ensure the incoming packet is allowed by ccsds
    	if (!pvn.has_value()) {
    		ccsdsLogNotice(TxRx::Tx, NotificationType::TypeServiceChannelNotif, ServiceChannelNotification::INVALID_PACKET_PVN);
    	...
    	if (pvn.value() == DefsAndUtils::PacketVersionNumber::SPACE_PACKET) {
    		// length checks
			if (packetSource.size() < (DefsAndUtils::SpacePacketPrimaryHeaderLength + 1) ||
				packetSource.size() > DefsAndUtils::MaxExpectedSpacePacketSize) {
				ccsdsLogNotice(TxRx::Tx, NotificationType::TypeServiceChannelNotif, ServiceChannelNotification::INVALID_LENGTH);
				return etl::unexpected(ServiceChannelNotification::INVALID_LENGTH);
			}

    		if (packetSource.size() != DefsAndUtils::getSpacePacketLength(packetSource)) {
    			ccsdsLogNotice(TxRx::Tx, NotificationType::TypeServiceChannelNotif, ServiceChannelNotification::INVALID_LENGTH);
    			return etl::unexpected(ServiceChannelNotification::INVALID_LENGTH);
    		}
    	} else if (pvn.value() == DefsAndUtils::PacketVersionNumber::ENCAPSULATION_PACKET) {
    		// TODO
    		return {};
    	}

    	if (!ChannelsInterface::pushTmPacketVirtualChannelSpaceSegment(vcChanVariant, packetSource).has_value()) {
    ...
    void ServiceChannelSpaceSegment::generateIdleSpacePacket(VirtualChannelSpaceSegmentVariant &vChanVariant,
	    ...
	    for (uint8_t i = 0; i < DefsAndUtils::SpacePacketPrimaryHeaderLength - 2; ++i) {
		    tmpData[i] = DefsAndUtils::IdleSpacePacketPrimaryHeader[i];
	    }
    ...
    etl::expected<void, ServiceChannelNotification> ServiceChannelSpaceSegment::allFramesGenerationRequestTM(
	    PhysicalChannel &physicalChannel,
	    MasterChannelSpaceSegmentVariant &mcChanVariant,
    ...
    // Packet Processing
    etl::expected<void, ServiceChannelNotification> ServiceChannelGroundSegment::storePacketTC(
	    etl::variant<VirtualChannelGroundSegmentVariant &, MAPChannelGroundSegmentVariant &> &channelVariant,
	    const DefsAndUtils::ServiceType serviceType,
	    const etl::span<uint8_t> &packetSource) {

    	if (serviceType == DefsAndUtils::ServiceType::TYPE_BC || serviceType == DefsAndUtils::ServiceType::TYPE_RESERVED) {
    	...
    	// push to queue
    	if (channelVariant.is_type<VirtualChannelGroundSegmentVariant>()) {
    		auto vcChanVariant = etl::get<VirtualChannelGroundSegmentVariant>(channelVariant);

    		if (!ChannelsInterface::pushPacketVirtualChannelGroundSegment(vcChanVariant, serviceType, packetSource).has_value()) {
    			ccsdsLogNotice(TxRx::Tx, NotificationType::TypeServiceChannelNotif, ServiceChannelNotification::PACKET_QUEUE_FULL);
    			return etl::unexpected(ServiceChannelNotification::PACKET_QUEUE_FULL);
    		}
    	} else { // MAP channel variant
		    auto mapChanVariant = etl::get<MAPChannelGroundSegmentVariant>(channelVariant);

    		if (!ChannelsInterface::pushPacketMAPChannelGroundSegment(mapChanVariant, serviceType, packetSource).has_value()) {
    			ccsdsLogNotice(TxRx::Tx, NotificationType::TypeServiceChannelNotif, ServiceChannelNotification::PACKET_QUEUE_FULL);
    			return etl::unexpected(ServiceChannelNotification::PACKET_QUEUE_FULL);
    		}
    	}
	...
	etl::expected<void, ServiceChannelNotification> ServiceChannelGroundSegment::blocking(
	const PhysicalChannel& physicalChannel,
	...
	const DefsAndUtils::ServiceType serviceType,
	const uint16_t securityHeaderLength,
	const uint16_t securityTrailerLength,
	bool& ranOutOfPacketsFlag,
	etl::optional<uint16_t>& packetLengthForSegmentation) {

    	const BaseMasterChannel *baseMcChanPtr = ChannelsInterface::upcastToBase(mcChanVariant);
    	const BaseVirtualChannel *baseVchanPtr = ChannelsInterface::upcastToBase(vcChanVariant);
    	BaseMAPChannel *baseMapChanPtr;

    	if (mapChanVariant.has_value()) {
    		baseMapChanPtr = ChannelsInterface::upcastToBase(mapChanVariant.value());
    	}

    	const uint16_t maxFrameLengthTc = physicalChannel.getMaxTcFrameLength();
    	const uint8_t trailerLength = physicalChannel.getFrameErrorControlFieldPresent() *
									DefsAndUtils::ErrorControlFieldSize + securityTrailerLength;
    	const uint8_t segmentHeaderLength = mapChanVariant.has_value() ? DefsAndUtils::TcSegmentHeaderSize : 0;
    	const uint16_t maxTransferFrameDataFieldLength = maxFrameLengthTc -
														 DefsAndUtils::TcPrimaryHeaderSize -
														 segmentHeaderLength -
    	...
    	// note: checking if there is a max sized block available is not optimal
    	//       (since the frame might turn out to be smaller), but it is hard to know beforehand.
    	if (!ChannelsInterface::hasCapacityForFrameDataMasterChannelGroundSegment(
			mcChanVariant, 1, maxFrameLengthTc)) {
    		notif = ServiceChannelNotification::NOT_ENOUGH_SPACE_IN_MASTER_COPY_OR_MEMORY_POOL;
		}

    	if (mapChanVariant.has_value()) {
    		if (ChannelsInterface::frameListAvailableMapChannelGroundSegment(mapChanVariant.value()) == 0) {
    			notif = ServiceChannelNotification::FRAME_LIST_FULL;
    		}
    	} else if (ChannelsInterface::frameListAvailableVirtualChannelGroundSegment(vcChanVariant) == 0) {
    		notif = ServiceChannelNotification::FRAME_LIST_FULL;
    	}
    	...
    	// Helper function for popping packet lengths. Returns false when there are no more packets to pop.
	    auto updatePacketLength = [](
		    VirtualChannelGroundSegmentVariant& vcChanVar, etl::optional<MAPChannelGroundSegmentVariant&> mapChanVar,
		    const DefsAndUtils::ServiceType servType, uint16_t &packetLen) -> bool {
		    if (!mapChanVar.has_value()) {
			    if (auto expectedPacketLength = ChannelsInterface::popPacketLengthVirtualChannelGroundSegment(
				    vcChanVar, servType); expectedPacketLength.has_value()) {
				    packetLen = expectedPacketLength.value();
				    return true;
			    }
			    return false;
		    } else {
			    if (auto expectedPacketLength = ChannelsInterface::popPacketLengthMAPChannelGroundSegment(
				    mapChanVar.value(), servType); expectedPacketLength.has_value()) {
				    packetLen = expectedPacketLength.value();
				    return true;
			    }
			    return false;
    	...
    	// cumlength
    	// while (updateLength)
    	//     if cumlength + new length <= max data field len
    	//          add to queue
    	//     else
    	//        if new length <= max data field len
    	//             it can fit alone in a new frame, so just

    	// pop the first packet, if there is one, and create the frame
    	uint16_t packetLength;
    	uint8_t tmpframeData[maxFrameLengthTc];
    	TransferFrameTC* frameTcPtr;
	    if (updatePacketLength(vcChanVariant, mapChanVariant, serviceType, packetLength)) {
		    frameData = ChannelsInterface::allocateBlockFromMemPoolMasterChannelGroundSegment(mcChanVariant, maxFrameLengthTc).value();

	    	if (mapChanVariant.has_value()) {
			    ChannelsInterface::popPacketSegmentMAPChannelGroundSegment(
				    mapChanVariant.value(), serviceType,
				    frameData + DefsAndUtils::TcPrimaryHeaderSize + segmentHeaderLength + securityHeaderLength,
				    packetLength);
	    	} else {
	    		ChannelsInterface::popPacketSegmentVirtualChannelGroundSegment(
					vcChanVariant, serviceType,
					frameData + DefsAndUtils::TcPrimaryHeaderSize + segmentHeaderLength + securityHeaderLength,
					packetLength);
	    	}

	    	frameTcPtr = etl::get<TransferFrameTC*>(ChannelsInterface::addFrameObjectToMasterCopyBufferMasterChannelGroundSegment(
	    		mcChanVariant,
				 TransferFrameTC(
				 	frameData,
				 	serviceType,
	    ...
	    while (updatePacketLength(vcChanVariant, packetLength, serviceType)) {
    		// TODO decide if the task needs to be sent to segmentation

    		if (packetLength > maxTransferFrameDataFieldLength) {
    			// this packet must be sent to segmentation
    			packetLengthForSegmentation = packetLength;
    			return {};
    		} else {
    			currentDataFieldLength += packetLength;
    		}
	...
	etl::expected<void, ServiceChannelNotification>
    ServiceChannelGroundSegment::blocking(uint16_t maxTransferFrameDataFieldLength, uint16_t packetLength,
                               uint8_t vcid, uint8_t mapid, ServiceType serviceType) {
	...
	etl::expected<void, ServiceChannelNotification> ServiceChannelGroundSegment::segmentation(
	PhysicalChannel& physicalChannel,
	MasterChannelGroundSegmentVariant &mcChanVariant,
	VirtualChannelGroundSegmentVariant &vcChanVariant,
	etl::optional<MAPChannelGroundSegmentVariant&> &mapChanVariant,
	...
	const uint16_t securityTrailerLength) {
		...
		const uint8_t numberOfMaxLenTransferFrames = packetLength / maxTransferFrameDataFieldLength;
		const bool nonMaxLenFrameNeeded = (packetLength % maxTransferFrameDataFieldLength) ? true : false;
		const uint16_t numberOfNewOctets = numberOfMaxLenTransferFrames * maxFrameLengthTc +
		                                   nonMaxLenFrameNeeded * (
			                                   DefsAndUtils::TcPrimaryHeaderSize + segmentHeaderLength +
			                                   securityHeaderLength + (
				                                   packetLength - numberOfMaxLenTransferFrames *
				                                   maxTransferFrameDataFieldLength) + trailerLength);

		// Ensure there is enough space for the new frames. If there is not, then the operation must be halted,
		// and the packet length returned back to the respective queue.
		etl::optional<ServiceChannelNotification> notif;
		if (!ChannelsInterface::hasCapacityForFrameDataMasterChannelGroundSegment(
			mcChanVariant, numberOfMaxLenTransferFrames + nonMaxLenFrameNeeded, numberOfNewOctets)) {
			notif = ServiceChannelNotification::NOT_ENOUGH_SPACE_IN_MASTER_COPY_OR_MEMORY_POOL;
		}

		if (mapChanVariant.has_value()) {
			if (ChannelsInterface::frameListAvailableMapChannelGroundSegment(mapChanVariant.value()) <
			    numberOfMaxLenTransferFrames + nonMaxLenFrameNeeded) {
				notif = ServiceChannelNotification::FRAME_LIST_FULL;
				// return the packet length back to the queue
			}
		} else if (ChannelsInterface::frameListAvailableVirtualChannelGroundSegment(vcChanVariant) <
		           numberOfMaxLenTransferFrames + nonMaxLenFrameNeeded) {
			notif = ServiceChannelNotification::FRAME_LIST_FULL;
		}
		...
		if (notif.has_value()) {
			// return the packet length back to the queue
			if (mapChanVariant.has_value()) {
				etl::visit([&](auto &mapChan) {
					if (serviceType == DefsAndUtils::ServiceType::TYPE_AD) {
						mapChan.packetLengthBufferTypeAD.push_front(packetLength);
					} else {
						mapChan.packetLengthBufferTypeBD.push_front(packetLength);
					}
				}, mapChanVariant.value());
			} else {
				etl::visit([&](auto &vcChan) {
					if (serviceType == DefsAndUtils::ServiceType::TYPE_AD) {
						vcChan.packetLengthBufferTcTypeAD.push_front(packetLength);
					} else {
						vcChan.packetLengthBufferTcTypeBD.push_front(packetLength);
					}
				}, vcChanVariant);
			}
    	...
    	// new frames creation
    	uint16_t remainingPacketSegmentLength = packetLength;
    	uint16_t frameLength = maxFrameLengthTc;
    	for (uint8_t i = 0; i < numberOfMaxLenTransferFrames + nonMaxLenFrameNeeded; ++i) {
			if ((i == numberOfMaxLenTransferFrames + nonMaxLenFrameNeeded - 1) && nonMaxLenFrameNeeded) {
				// last frame most likely is not the max size
				frameLength = DefsAndUtils::TcPrimaryHeaderSize + securityHeaderLength + segmentHeaderLength + remainingPacketSegmentLength + trailerLength;
			}

    		uint8_t* frameData = ChannelsInterface::allocateBlockFromMemPoolMasterChannelGroundSegment(mcChanVariant, frameLength).value();

    		// place packet segment to frame data field
			if (mapChanVariant.has_value()) {
				ChannelsInterface::popPacketSegmentMAPChannelGroundSegment(
					mapChanVariant.value(), serviceType,
					frameData + DefsAndUtils::TcPrimaryHeaderSize + segmentHeaderLength + securityHeaderLength,
					maxTransferFrameDataFieldLength);
			} else {
				ChannelsInterface::popPacketSegmentVirtualChannelGroundSegment(
					vcChanVariant, serviceType,
					frameData + DefsAndUtils::TcPrimaryHeaderSize + segmentHeaderLength + securityHeaderLength,
    		...
    		DefsAndUtils::SequenceFlag seqFlag = DefsAndUtils::SequenceFlag::SegmentationMiddle;
    		if (i == 0) {
    			seqFlag = DefsAndUtils::SequenceFlag::SegmentationStart;
    		} else if (i == numberOfMaxLenTransferFrames + nonMaxLenFrameNeeded - 1) {
    			seqFlag = DefsAndUtils::SequenceFlag::SegmentationEnd;
    		}

			const auto frameTcPtr = etl::get<TransferFrameTC *>(
				ChannelsInterface::addFrameObjectToMasterCopyBufferMasterChannelGroundSegment(
					...
					TransferFrameTC(
						...
						DefsAndUtils::extractScidFromMcid(baseMcChanPtr->getMscid()),
						frameLength,
						baseVchanPtr->getSegmentHeaderTCPresent(),
						seqFlag,
						mapChanVariant.has_value() ?
						DefsAndUtils::extractMapidFromGmapid(baseMapChanPtr->getGmapid()) : 0)
					).value());

			if (mapChanVariant.has_value()) {
				ChannelsInterface::pushFrameMapChannelGroundSegment(mapChanVariant.value(), frameTcPtr);
			} else {
				ChannelsInterface::pushFrameVirtualChannelGroundSegment(vcChanVariant, frameTcPtr);
			}

    		frameTcPtr->updateProcessingStage(DefsAndUtils::TcFrameProcessingStage::PROCESSED_BY_FOP);
    		remainingPacketSegmentLength -= maxTransferFrameDataFieldLength;
    	}
    ...
    // SDLS Processing
    etl::expected<void, ServiceChannelNotification> ServiceChannelGroundSegment::applySDLSSecurityTxTC(uint8_t vcid, uint8_t mapid) {
	    if (masterChannel.virtualChannels.find(vcid) == masterChannel.virtualChannels.end()) {
		    ccsdsLogNotice(TxRx::Tx, NotificationType::TypeServiceChannelNotif, ServiceChannelNotification::INVALID_VC_ID);
		    return etl::unexpected(ServiceChannelNotification::INVALID_VC_ID);
	    }
