...
TEST_CASE("CCSDS TC Channel Model") {
	...
	master_channel.addVC(3, true, 1024, 20, true, 32, 32, 32, true, true, map_channels);

    CHECK(master_channel.virtChannels.at(3).VCID == 0x03);
	PhysicalChannel physical_channel = PhysicalChannel(TmTransferFrameSize, TcErrorControlFieldExists,
                                                       100, 50, 20000, 5);
...
TEST_CASE("MAPP blocking") {
    ...
    MasterChannel master_channel = MasterChannel(true, 0);
	master_channel.addVC(3, true, 8, 20, true, 32, 32, 32, true, true, map_channels);
