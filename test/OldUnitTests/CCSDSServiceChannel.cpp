...
//         master_channel.addVC(0, true, 128, true, true, true, 2, 2, true, true, 8, true,
//                              SynchronizationFlag::OCTET_SYNCHRONIZED_FORWARD_ORDERED, 255, 10, 10, 3,
//                              map_channels);
//
//         master_channel.addVC(1, false, 128, false, false, false, 2, 2, true, true, true, true,
//                              SynchronizationFlag::OCTET_SYNCHRONIZED_FORWARD_ORDERED, 20, 3, 3, 3);
//
//         master_channel.addVC(2, false, 128, false, false, false, 2, 2, false, true, true, true,
//                              SynchronizationFlag::OCTET_SYNCHRONIZED_FORWARD_ORDERED, 20, 3, 3, 3,
//                              map_channels);
//
...
//         const TransferFrameTC *frame_a = serv_channel.frontFrameBeforeSDLSProcessing(0).second;
//
//         CHECK(err == ServiceChannelNotification::NO_SERVICE_EVENT);
//         CHECK(serv_channel.availableFramesBeforeSDLSProcessing(0) == MaxReceivedUnprocessedTxTcInVirtBuffer - 2);
//
...
//         CHECK(err == ServiceChannelNotification::NO_SERVICE_EVENT);
//         CHECK(serv_channel.frontFrameBeforeSDLSProcessing(0).second == frame_a);
//
...
//         err = serv_channel.vcGenerationRequestTxTC(0);
//         CHECK(err == ServiceChannelNotification::NO_SERVICE_EVENT);
//         //CHECK(serv_channel.frontFrameBeforeSDLSProcessing(0).second == frame_c);
//         //CHECK(serv_channel.availableFramesBeforeSDLSProcessing(0) == MaxReceivedUnprocessedTxTcInVirtBuffer - 1U);
...
//         CHECK(err == ServiceChannelNotification::NO_SERVICE_EVENT);
//         CHECK(serv_channel.getAvailableBeforeVcReceptionRxTC(0) == MaxReceivedTxTcInWaitQueue);
//         CHECK(serv_channel.getAvailableInFramesAfterVCReceptionTypeADRxTC(0) ==
//               MaxReceivedRxTcInVirtualChannelBuffer - 1);
//
...
//         CHECK(err == ServiceChannelNotification::NO_SERVICE_EVENT);
//         CHECK(serv_channel.getAvailableBeforeVcReceptionRxTC(1) == MaxReceivedTxTcInWaitQueue);
//         CHECK(serv_channel.getAvailableInFramesAfterVCReceptionTypeADRxTC(1) ==
//               MaxReceivedRxTcInVirtualChannelBuffer - 1);
//
//         // SDLS Processing
...
//         */
//
//         uint8_t valid_frame_TM[128] = {0x00, 0x01, 0x00, 0x03, 0x04, 0xA2, 0xB3, 0x1F, 0xD6, 0xA2, 0xB3, 0x1F, 0x7B,
//                                        0x7C};
//         uint8_t len = 14;
...
//
//         uint8_t invalid_vcid_TM[128] = {0x00, 0x0F, 0x00, 0x03, 0x04, 0xA2, 0xB3, 0x1F, 0xD6, 0xA2, 0xB3, 0x1F, 0x7B,
//                                         0x7C};
//         uint8_t invalid_crc_TM[128] = {0x00, 0x01, 0x00, 0x03, 0x04, 0xA2, 0xB3, 0x5B, 0x54, 0xA2, 0xB3, 0x1F, 0xD6,
//                                        0x01};
//
...
//         CHECK(err == ServiceChannelNotification::NO_SERVICE_EVENT);
//         //CHECK(serv_channel.availableFramesBeforeSDLSProcessing(2) == MaxReceivedUnprocessedTxTcInVirtBuffer);
...
//
//         MasterChannel master_channel = MasterChannel();
//         master_channel.addVC(0, false, 128, true, true, true, 2, 2, true, true, 8, true,
//                              SynchronizationFlag::OCTET_SYNCHRONIZED_FORWARD_ORDERED, 255, 10, 10, 3,
//                              map_channels);
//         master_channel.addVC(1, false, 128, false, true, true, 2, 2, true, true, 8, true,
//                              SynchronizationFlag::OCTET_SYNCHRONIZED_FORWARD_ORDERED, 255, 10, 10, 3,
//                              map_channels);
//         master_channel.addVC(2, false, 128, true, false, true, 2, 2, true, true, 8, true,
//                              SynchronizationFlag::OCTET_SYNCHRONIZED_FORWARD_ORDERED, 255, 10, 10, 3,
//                              map_channels);
//
//
...
//             uint16_t upperHalf = static_cast<uint16_t >(transferFrame->getFrameData()[TmPrimaryHeaderSize +
//                                                                                       sizeof(packet1) +
//                                                                                       sizeof(packet2) +
//                                                                                       sizeof(packet3) +
//                                                                                       PacketPrimaryHeaderLength - 2]);
//             uint16_t lowerHalf = static_cast<uint16_t >(transferFrame->getFrameData()[TmPrimaryHeaderSize +
//                                                                                       sizeof(packet1) +
//                                                                                       sizeof(packet2) +
//                                                                                       sizeof(packet3) +
//                                                                                       PacketPrimaryHeaderLength - 1]);
...
//         master_channel.addVC(1, false, 128, true, true, true, 2, 2, true, true, 8, true,
//                              SynchronizationFlag::OCTET_SYNCHRONIZED_FORWARD_ORDERED, 255, 10, 10, 3);
//
//         ServiceChannel serv_channel = ServiceChannel(master_channel, phy_channel_fop, SecurityAssociation(),
//                                                      SecurityAssociation());
...
//             CHECK(err == NO_SERVICE_EVENT);
//
//             const TransferFrameTC *transferFrame = serv_channel.frontFrameBeforeSDLSProcessing(1).second;
//
//             CHECK(transferFrame->getServiceType() == ServiceType::TYPE_BD);
//             CHECK(transferFrame->getFrameLength() ==
//                   TcPrimaryHeaderSize + sizeof(packet1) + sizeof(packet2) + sizeof(packet3) + ErrorControlFieldSize);
//             for (uint8_t i = TcPrimaryHeaderSize;
//                  i < sizeof(packet1) + sizeof(packet2) + sizeof(packet3) + TcPrimaryHeaderSize; i++) {
...
//             CHECK(err == NO_SERVICE_EVENT);
//
//             const etl::list<TransferFrameTC *, MaxReceivedUnprocessedTxTcInVirtBuffer> &buffer = serv_channel.getFramesBeforeSDLSProcessing(
//                     0);
...
//             CHECK(((frame1->getSegmentationHeader() & 0xC0) >> 6) == SequenceFlags::SegmentationStart);
...
//             CHECK(((frame2->getSegmentationHeader() & 0xC0) >> 6) == SequenceFlags::SegmentationMiddle);
...
//             CHECK(((frame3->getSegmentationHeader() & 0xC0) >> 6) == SequenceFlags::SegmentationEnd);
...
//
//         MasterChannel master_channel = MasterChannel();
//         master_channel.addVC(0, false, 128, true, true, true, 2, 2, false, false, 0, 8,
//                              SynchronizationFlag::OCTET_SYNCHRONIZED_FORWARD_ORDERED, 255, 10, 10, 3,
//                              map_channels);
//
//         ServiceChannel serv_channel = ServiceChannel(master_channel, phy_channel_fop, SecurityAssociation(),
...
//
//         ServiceChannelNotification err;
//         uint8_t frame1[] = {0x00, 0xAC, 0x00, 0x0A, 0x00, 0x00, 0x00, 0x1C, 0xD3, 0x8C};
//         uint8_t frame2[] = {0x00, 0xAC, 0x00, 0x0A, 0x03, 0x00, 0x00, 0x1C, 0xD3, 0x8C};
//         uint8_t frame3[] = {0x00, 0xAC, 0x00, 0x0A, 0x12, 0x00, 0x00, 0x1C, 0xD3, 0x8C};
//         serv_channel.storeFrameRxTC(frame1, 10);
//         serv_channel.storeFrameRxTC(frame2, 10);
...
//
//         MasterChannel master_channel = MasterChannel();
//         master_channel.addVC(0, false, 128, true, true, true, 2, 2, false, false, 0, true,
//                              SynchronizationFlag::OCTET_SYNCHRONIZED_FORWARD_ORDERED, 255, 10, 10, 3,
//                              map_channels);
//         master_channel.addVC(1, false, 128, true, true, true, 2, 2, false, false, 0, true,
//                              SynchronizationFlag::OCTET_SYNCHRONIZED_FORWARD_ORDERED, 255, 10, 10, 3,
//                              map_channels);
//
//         ServiceChannel serv_channel = ServiceChannel(master_channel, phy_channel_fop, SecurityAssociation(),
